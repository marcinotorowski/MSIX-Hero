...
{
    public class AppxVolumeViaPowerShellService : IAppxVolumeService
    {
        ...
        public async Task<AppxVolume> GetDefault(CancellationToken cancellationToken = default, IProgress<ProgressData> progress = default)
        {
            ...
            appxVolume.Capacity = (ulong)drive.TotalSize;
            appxVolume.AvailableFreeSpace = (ulong)drive.AvailableFreeSpace;
        ...
        public async Task<List<AppxVolume>> GetAll(CancellationToken cancellationToken = default, IProgress<ProgressData> progress = default)
        {
            ...
            foreach (var drive in list.Where(c => c.PackageStorePath.IndexOf(":\\", StringComparison.Ordinal) == 1))
            {
                ...
                if (matchingDrive?.IsReady == true)
                {
                    ...
                    drive.AvailableFreeSpace = (ulong)matchingDrive.AvailableFreeSpace;
        ...
        public Task MovePackageToVolume(AppxVolume volume, string packageFullName, CancellationToken cancellationToken = default, IProgress<ProgressData> progress = default)
        {
            if (volume == null)
            {
            ...
            return this.MovePackageToVolume(volume.Name, packageFullName, cancellationToken, progress);
        }
