...
{
    internal class SetVolumeFilterClientExecutor : CommandExecutor
    {
        private readonly SetVolumeFilter action;

        public SetVolumeFilterClientExecutor(SetVolumeFilter action, IWritableApplicationStateManager stateManager) : base(action, stateManager)
        {
            this.action = action;
        }
            ...
            foreach (var item in state.Volumes.VisibleItems)
            {
                if (!string.IsNullOrEmpty(action.SearchKey))
                {
                    if (
                        ...
                        item.DiskLabel?.IndexOf(action.SearchKey, StringComparison.OrdinalIgnoreCase) == -1)
                    {
                        toHide.Add(item);
                    }
                }
