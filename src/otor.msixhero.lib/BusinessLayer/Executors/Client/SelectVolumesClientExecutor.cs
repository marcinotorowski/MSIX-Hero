...
{
    internal class SelectVolumesClientExecutor : CommandWithOutputExecutor<List<AppxVolume>>
    {
        private readonly SelectVolumes action;

        public SelectVolumesClientExecutor(SelectVolumes action, IWritableApplicationStateManager stateManager) : base(action, stateManager)
        {
            this.action = action;
        }

        public override Task<List<AppxVolume>> ExecuteAndReturn(IInteractionService interactionService, CancellationToken cancellationToken = default, IProgress<ProgressData> progressReporter = default)
        {
