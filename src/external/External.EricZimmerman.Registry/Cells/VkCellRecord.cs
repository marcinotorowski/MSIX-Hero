...
namespace External.EricZimmerman.Registry.Cells
{
    ...
    public class VkCellRecord : ICellTemplate, IRecordBase
    {
        ...
        public VkCellRecord(int recordSize, long relativeOffset, int minorVersion, IRegistry registryHive)
        {
            ...
            this.DataOffsets = new HashSet<ulong>();

            this.OffsetToData = BitConverter.ToUInt32(this.RawBytes, 0x0c);

            this._dataLengthInternal = this.DataLength;

            //if the high bit is set, data lives in the field used to typically hold the OffsetToData Value
            this._dataIsResident = (this._dataLengthInternal & DwordSignMask) == DwordSignMask;
            ...
            this._internalDataOffset = 4;

            if (this._dataIsResident)
            {
                //normalize the data for future use
                this._dataLengthInternal = this._dataLengthInternal - 0x80000000;
            ...
            else if (this.IsFree)
            {
                ...
                try
                {
                    ...
                    var sizeBytes = this._registryHive.ReadBytesFromHive(4096 + this.OffsetToData, 4);
            ...
            var dataTypeInternal = this.DataTypeRaw;
            ...
            this.DataType = (DataTypeEnum) dataTypeInternal;
        }
            ...
            get
            {
                ...
                else
                {
                    ...
                    if (this.IsFree)
                    {
                        try
                        {
                            datablockSizeRaw = this._registryHive.ReadBytesFromHive(4096 + this.OffsetToData, 4);
                        }
                        catch (Exception)
                        {
                            //crazy things can happen in IsFree records
                    ...
                    this.DataOffsets.Add(this.OffsetToData);
                    ...
                    if (this.IsFree && dataBlockSize > this.DataLength * 100)
                    {
                        ...
                        dataBlockSize = (int) (Math.Ceiling((double) this.DataLength / 8) * 8) + 32;
                    }

                    if (this.IsFree && dataBlockSize == this.DataLength)
                    {
                        dataBlockSize += 4;
                    }
                    ...
                    if (this.IsFree)
                    {
                        try
                        {
                            datablockRaw = this._registryHive.ReadBytesFromHive(4096 + this.OffsetToData, dataBlockSize);
                        }
                        catch (Exception)
                        {
                            //crazy things can happen in IsFree records
                    ...
                    if (this._dataLengthInternal > 16344 && this._minorVersion > 3)
                    {
                        ...
                        var db = new DbListRecord(datablockRaw, 4096 + this.OffsetToData);
                        ...
                        datablockSizeRaw = this._registryHive.ReadBytesFromHive(4096 + db.OffsetToOffsets, 4);
                        ...
                        datablockRaw = this._registryHive.ReadBytesFromHive(4096 + db.OffsetToOffsets,
                            Math.Abs(dataBlockSize));

                        //datablockRaw now contains our list of pointers to fragmented Data

                        //make a place to reassemble things
                        var bigDataRaw = new ArrayList((int) this._dataLengthInternal);
                        ...
                        for (var i = 1; i <= db.NumberOfEntries; i++)
                        {
                            ...
                            this.DataOffsets.Add(os);

                            var tempDataBlockSizeRaw = this._registryHive.ReadBytesFromHive(4096 + os, 4);
                            ...
                            try
                            {
                                ...
                                //get our data block
                                var tempDataRaw =
                                    this._registryHive.ReadBytesFromHive(4096 + os, Math.Abs(tempdataBlockSize));
        ...
        public byte[] Padding
        {
            get
            {
                var paddingOffset = 0x18 + this.NameLength;
        ...
        // public properties...
        public uint DataLength => BitConverter.ToUInt32(this.RawBytes, 0x08);

        public DataTypeEnum DataType { get; set; }

        //we need to preserve the datatype as it exists (so we can see unsupported types easily)
        public uint DataTypeRaw => BitConverter.ToUInt32(this.RawBytes, 0x10) & DevpropMaskType;

        public ushort NameLength => BitConverter.ToUInt16(this.RawBytes, 0x06);

        //TODO IsTombstone?
        /// <summary>
        ///     Used to determine if the name is stored in ASCII (> 0) or Unicode (== 0)
        /// </summary>
        public ushort NamePresentFlag => BitConverter.ToUInt16(this.RawBytes, 0x14);
            ...
            get
            {
                ...
                try
                {
                    ...
                    if (this.IsFree)
                    {
                        ...
                        if (localDbl.Length < this._dataLengthInternal)
                        {
                    ...
                    switch (this.DataType)
                    {
                        case DataTypeEnum.RegFileTime:
                            var ts = BitConverter.ToUInt64(localDbl, this._internalDataOffset);
                        ...
                        case DataTypeEnum.RegSz:
                            var tempVal = Encoding.Unicode.GetString(localDbl, this._internalDataOffset,
                                (int) this._dataLengthInternal);
                        ...
                        case DataTypeEnum.RegFullResourceDescription:
                            val =
                                new ArraySegment<byte>(localDbl, this._internalDataOffset,
                                        (int) Math.Abs(this._dataLengthInternal))
                                    .ToArray();
                            break;

                        case DataTypeEnum.RegDword:
                            val = this._dataLengthInternal == 4 ? BitConverter.ToUInt32(localDbl, 0) : 0;
                            break;
                        ...
                        case DataTypeEnum.RegQword:
                            val = this._dataLengthInternal == 8 ? BitConverter.ToUInt64(localDbl, this._internalDataOffset) : 0;
                            break;
                        ...
                        case DataTypeEnum.RegLink:
                            val =
                                Encoding.Unicode.GetString(localDbl, this._internalDataOffset, (int) this._dataLengthInternal)
                                    .Replace("\0", " ")
                                    .Trim();
                            break;

                        default:
                            this.DataType = DataTypeEnum.RegUnknown;
                            val = localDbl;
                            break;
                    }
                }
                ...
                catch (Exception ex)
                {
                    ...
                    else
                    {
                        ...
                        logger.Error(
                            $"Error getting ValueData for {this.ValueName}. Error message: {ex.Message}, Stack: {ex.StackTrace}");
                        val = localDbl;
                        //throw;
                    }
                }
        ...
        public byte[] ValueDataRaw
        {
            get
            {
                ...
                if (this._dataLengthInternal + this._internalDataOffset > this.DataBlockRaw.Length)
                {
                    //we don't have enough data to copy, so take what we can get

                    if (this.DataBlockRaw.Length > 0)
                    {
                        try
                        {
                            return new ArraySegment<byte>(this.DataBlockRaw, this._internalDataOffset,
                                this.DataBlockRaw.Length - this._internalDataOffset).ToArray();
                        }
                        catch (Exception)
                        {
                ...
                {
                        return new ArraySegment<byte>(this.DataBlockRaw, this._internalDataOffset,
                            (int) this._dataLengthInternal).ToArray();
                }
        ...
        public byte[] ValueDataSlack
        {
            get
            {
                var dbRaw = this.DataBlockRaw;

                if (dbRaw.Length > this._dataLengthInternal + this._internalDataOffset)
                {
                    var slackStart = (int) (this._dataLengthInternal + this._internalDataOffset);
                    var slackLen = dbRaw.Length - slackStart;

                    return
                        new ArraySegment<byte>(this.DataBlockRaw, slackStart, slackLen).ToArray();
                }
        ...
        public string ValueName
        {
            get
            {
                ...
                {
                    if (this.NamePresentFlag > 0)
                    {
                        if (this.IsFree)
                        {
                            //make sure we have enough data
                            if (this.RawBytes.Length >= this.NameLength + 0x18)
                            {
                                valName = Encoding.GetEncoding(1252).GetString(this.RawBytes, 0x18, this.NameLength);
                            }
                        }
                        else
                        {
                    ...
                    else
                    {
                        ...
                        var valString = BitConverter.ToString(this.RawBytes, 0x18, this.NameLength);
                        ...
                        if (foundMatch)
                        {
                            ...
                            valName = Encoding.Unicode.GetString(this.RawBytes, 0x18, this.NameLength);
                        }
                    }
                }
        ...
        public int Size => BitConverter.ToInt32(this.RawBytes, 0);
        ...
        public override string ToString()
        {
            ...
            sb.AppendLine($"Data Type raw: 0x{this.DataTypeRaw:X}");
            sb.AppendLine();
            sb.AppendLine($"Is Free: {this.IsFree}");

            sb.AppendLine();

            sb.AppendLine($"Data Length: 0x{this.DataLength:X}");
            sb.AppendLine($"Offset To Data: 0x{this.OffsetToData:X}");

            sb.AppendLine();

            sb.AppendLine($"Name Length: 0x{this.NameLength:X}");
            sb.AppendLine($"Name Present Flag: 0x{this.NamePresentFlag:X}");
            ...
            sb.AppendLine($"Value Name: {this.ValueName}");
            sb.AppendLine($"Value Type: {this.DataType}");
            ...
            switch (this.DataType)
            {
                ...
                case DataTypeEnum.RegMultiSz:
                case DataTypeEnum.RegLink:
                    sb.AppendLine($"Value Data: {this.ValueData}");
                    break;
                ...
                case DataTypeEnum.RegResourceRequirementsList:
                case DataTypeEnum.RegFullResourceDescription:
                    sb.AppendLine($"Value Data: {BitConverter.ToString((byte[]) this.ValueData)}");
                    break;

                case DataTypeEnum.RegFileTime:
                    if (this.ValueData != null)
                    {
                        var dto = (DateTimeOffset) this.ValueData;
                ...
                case DataTypeEnum.RegDword:
                case DataTypeEnum.RegQword:
                    sb.AppendLine($"Value Data: {this.ValueData:N}");
                    break;
                default:
                    sb.AppendLine($"Value Data: {BitConverter.ToString((byte[]) this.ValueData)}");
                    break;
            ...
            if (this.ValueDataSlack != null)
            {
                sb.AppendLine($"Value Data Slack: {BitConverter.ToString(this.ValueDataSlack, 0)}");
            }
