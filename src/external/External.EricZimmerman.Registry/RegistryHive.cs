    ...
    public class RegistryHive : RegistryBase
    {
            ...
            ref int valueCount)
        {
            ...
            foreach (var subkey in key.SubKeys)
            {
                ...
                this.DumpKeyCommonFormat(subkey, sw, ref keyCount, ref valueCount);
            }
        }
        ...
        public byte[] ProcessTransactionLogs(List<TransactionLogFileInfo> logFileInfos, bool updateExistingData = false)
        {
            ...
            if (this.Header.PrimarySequenceNumber == this.Header.SecondarySequenceNumber)
            {
            ...
            foreach (var logFile in logFileInfos)
            {
                ...
                if (this.HiveType != transLog.HiveType)
                {
                    throw new Exception(
                        $"Transaction log contains a type ({transLog.HiveType}) that is different from the Registry hive ({this.HiveType})");
                }

                if (transLog.Header.PrimarySequenceNumber < this.Header.SecondarySequenceNumber)
                {
            ...
            if (logOne != null && logTwo != null)
            {
                ...
                this.Logger.Info("Two transaction logs found. Determining primary log...");
                ...
                this.Logger.Info($"Primary log: {firstLog.LogPath}, secondary log: {secondLog.LogPath}");
                ...
                if (this.Header.ValidateCheckSum() &&
                    firstLog.Header.PrimarySequenceNumber >= this.Header.SecondarySequenceNumber)
                {
                    this.Logger.Info($"Replaying log file: {firstLog.LogPath}");
                ...
                if (secondLog.Header.PrimarySequenceNumber == maximumSequenceNumber + 1 &&
                    secondLog.Header.PrimarySequenceNumber > this.Header.SecondarySequenceNumber)
                {
                    this.Logger.Info($"Replaying log file: {secondLog.LogPath}");
                    bytes = secondLog.UpdateHiveBytes(bytes);
            ...
            if (soloLog != null)
            {
                this.Logger.Info($"Single log file available: {soloLog.LogPath}");

                if (this.Header.ValidateCheckSum() && soloLog.Header.PrimarySequenceNumber >= this.Header.SecondarySequenceNumber)
                {
                    this.Logger.Info($"Replaying log file: {soloLog.LogPath}");
                    //we can replay the log
            ...
            if (wasUpdated)
            {
                ...
                var newchecksum = this.CalculateCheckSum(bytes);
            ...
            if (updateExistingData)
            {
                ...
                this.Initialize(); //reprocess header
      ...
      private int CalculateCheckSum(byte[] bytes)
      {
        ...
        public byte[] ProcessTransactionLogs(List<string> logFiles, bool updateExistingData = false)
        {
            ...
            return this.ProcessTransactionLogs(logfileInfos, updateExistingData);
        }

        //TODO this needs refactored to remove duplicated code
        private List<RegistryKey> GetSubKeysAndValues(RegistryKey key)
        {
            this._relativeOffsetKeyMap.Add(key.NkRecord.RelativeOffset, key);

            this._keyPathKeyMap.Add(key.KeyPath.ToLowerInvariant(), key);
            ...
            if (key.NkRecord.ClassCellIndex > 0)
            {
                ...
                var d = this.GetDataNodeFromOffset(key.NkRecord.ClassCellIndex);
                d.IsReferenced = true;
            ...
            if (key.NkRecord.ValueListCellIndex > 0)
            {
                ...
                var offsetList = this.GetDataNodeFromOffset(key.NkRecord.ValueListCellIndex);
            ...
            foreach (var valueOffset in key.NkRecord.ValueOffsets)
            {
                ...
                if (this.CellRecords.ContainsKey((long) valueOffset))
                {
                    var vc = this.CellRecords[(long) valueOffset];
                ...
                else
                {
                    if (valOffsetIndex < key.NkRecord.ValueListCount)
                    {
                        this.Logger.Warn($"An expected value was not found at offset 0x{valueOffset:X}. Key: {key.KeyPath}");
                    }
                }
...
//            sk.IsReferenced = true;
            ...
            if (this.CellRecords.ContainsKey(key.NkRecord.SecurityCellIndex))
            {
                var sk = this.CellRecords[key.NkRecord.SecurityCellIndex] as SkCellRecord;
                if (sk != null)
                {
                    sk.IsReferenced = true;
                }
            ...
            if (this.ListRecords.ContainsKey(key.NkRecord.SubkeyListsStableCellIndex) == false)
            {
                return keys;
            }
            ...
            var l = this.ListRecords[key.NkRecord.SubkeyListsStableCellIndex];
            ...
            switch (sig)
            {
                ...
                case LhSignature:
                    ...
                    foreach (var offset in lxRecord.Offsets)
                    {
                        ...
                        if (this.CellRecords.ContainsKey(offset.Key) == false)
                        {
                            this.Logger.Warn($"NK record at relative offset 0x{offset.Key} missing! Skipping");
                            continue;
                        }

                        var cell = this.CellRecords[offset.Key];
                        ...
                        var sks = this.GetSubKeysAndValues(tempKey);
                ...
                case RiSignature:
                    ...
                    foreach (var offset in riRecord.Offsets)
                    {
                        ...
                        var tempList = this.ListRecords[offset];
                        ...
                        if (tempList.Signature == "li")
                        {
                            ...
                            foreach (var offset1 in sk3.Offsets)
                            {
                                ...
                                var cell = this.CellRecords[offset1];
                        ...
                        else
                        {
                            ...
                            foreach (var offset3 in lxRecord2.Offsets)
                            {
                                ...
                                //            offset3.Key);
                                var cell = this.CellRecords[offset3.Key];
                ...
                case LiSignature:
                    ...
                    foreach (var offset in liRecord.Offsets)
                    {
                        ...
                        var cell = this.CellRecords[offset];
        ...
        public void ExportDataToCommonFormat(string outfile, bool deletedOnly)
        {
            ...
            using (var sw = new StreamWriter(outfile, false))
            {
                ...
                {
                    //dump active stuff
                    if (this.Root.LastWriteTime != null)
                    {
                        keyCount = 1;
                        sw.WriteLine("key|{0}|{1}|{2}|{3}", this.Root.NkRecord.IsFree ? "U" : "A",
                            this.Root.NkRecord.AbsoluteOffset,
                            this.Root.KeyPath, this.Root.LastWriteTime.Value.UtcDateTime.ToString("o"));
                    }

                    foreach (var val in this.Root.Values)
                    {
                        valueCount += 1;
                        sw.WriteLine(@"value|{0}|{1}|{2}|{3}|{4}|{5}", val.VkRecord.IsFree ? "U" : "A",
                            val.VkRecord.AbsoluteOffset, this.Root.KeyPath, val.ValueName, (int) val.VkRecord.DataType,
                            BitConverter.ToString(val.VkRecord.ValueDataRaw).Replace("-", " "));
                    }

                    this.DumpKeyCommonFormat(this.Root, sw, ref keyCount, ref valueCount);
                }

                var theRest = this.CellRecords.Where(a => a.Value.IsReferenced == false);
                ...
                foreach (var keyValuePair in theRest)
                {
                    try
                    {
                        ...
                        if (keyValuePair.Value.Signature == "nk")
                        {
                            ...
                            this.DumpKeyCommonFormat(key, sw, ref keyCountDeleted, ref valueCountDeleted);
                        }
                    }
                    catch (Exception ex)
                    {
                        this.Logger.Warn("There was an error exporting free record at offset 0x{0:X}. Error: {1}",
        ...
        public RegistryKey GetDeletedKey(long relativeOffset, long lastwritetimestampTicks)
        {
           ...
           var keys = this.DeletedRegistryKeys.Where(t => t.NkRecord.RelativeOffset == relativeOffset).ToList();
           ...
           if (!keys.Any())
           {
              var keys2 = this.CellRecords.Where(t => t.Value.RelativeOffset == relativeOffset).ToList();
           }
...
//            return null;
        ...
        public RegistryKey GetDeletedKey(string keyPath, string lastwritetimestamp)
        {
            ...
            //get a list that contains all matching root level unassociated keys
            var keys = this.DeletedRegistryKeys.Where(t => t.KeyPath == keyPath).ToList();
            ...
            if (!keys.Any())
            {
                keys = this.DeletedRegistryKeys.Where(t => t.KeyPath == segs[0]).ToList();
            }
            ...
            if (!keys.Any())
            {
                ...
                var newPath = $"{this.Root.KeyName}\\{keyPath}";

                keys = this.DeletedRegistryKeys.Where(t => t.KeyPath == newPath).ToList();
            }
        ...
        public RegistryKey GetKey(string keyPath)
        {
            ...
            if (this._keyPathKeyMap.ContainsKey(keyPath))
            {
                return this._keyPathKeyMap[keyPath];
            }

            //handle case where someone doesn't pass in ROOT keyname
            var newPath = $"{this.Root.KeyName}\\{keyPath}".ToLowerInvariant();

            if (this._keyPathKeyMap.ContainsKey(newPath))
            {
                return this._keyPathKeyMap[newPath];
            }
        ...
        {
            if (this._relativeOffsetKeyMap.ContainsKey(relativeOffset))
            {
                return this._relativeOffsetKeyMap[relativeOffset];
            }
        ...
        public bool ParseHive()
        {
            ...
            this.CellRecords = new Dictionary<long, ICellTemplate>();
            this.ListRecords = new Dictionary<long, IListTemplate>();

            this.DeletedRegistryKeys = new List<RegistryKey>();
            this.UnassociatedRegistryValues = new List<KeyValue>();

        this._keyPathKeyMap = new Dictionary<string, RegistryKey>();
         this._relativeOffsetKeyMap = new Dictionary<long, RegistryKey>();

            this.TotalBytesRead = 0;

            this.TotalBytesRead += 4096;
            ...
            var hiveLength = this.Header.Length + 0x1000;
            if (hiveLength < this.FileBytes.Length)
            {
                this.Logger.Debug("Header length is smaller than the size of the file.");
                hiveLength = (uint) this.FileBytes.Length;
            }
            ...
            {
                var hbinSize = BitConverter.ToUInt32(this.ReadBytesFromHive(offsetInHive + 8, 4), 0);

                if (hbinSize == 0)
                {
                    this.Logger.Debug("Found hbin with size 0 at absolute offset 0x{0:X}", offsetInHive);
                    // Go to end if we find a 0 size block (padding?)
                    offsetInHive = this.HiveLength();
                    continue;
                }

                var hbinSig = BitConverter.ToInt32(this.ReadBytesFromHive(offsetInHive, 4), 0);
...
//                    }
                ...
                var rawhbin = this.ReadBytesFromHive(offsetInHive, (int) hbinSize);

                try
                {
                    var h = new HBinRecord(rawhbin, offsetInHive - 0x1000, this.Header.MinorVersion, this.RecoverDeleted, this);
                        ...
                        //TODO change this to compare against constants?
                    {
                        switch (record.Signature)
                        {
                            ...
                            case "vk":
                                ...
                                this.CellRecords.Add(record.AbsoluteOffset - 4096, (ICellTemplate) record);
                                break;
                            ...
                            case "lf":
                                ...
                                //                        record.Signature, record.AbsoluteOffset);
                                this.ListRecords.Add(record.AbsoluteOffset - 4096, (IListTemplate) record);
                                break;
                        }
                    }

                    this.HBinRecordCount += 1;
                    this.HBinRecordTotalSize += hbinSize;
                }
                catch (Exception ex)
                {
                    this.Logger.Error(ex, $"Error processing hbin at absolute offset 0x{offsetInHive:X}.");
                }

                offsetInHive += hbinSize;
            }

            this.Logger.Debug("Initial processing complete. Building tree...");
            ...
            var rootNode =
                this.CellRecords.Values.OfType<NkCellRecord>()
                    .SingleOrDefault(
                        f => f.RelativeOffset == (long) this.Header.RootCellOffset);
            ...
            this.Logger.Debug("Found root node! Getting subkeys...");

            this.Root = new RegistryKey(rootNode, null);
            this.Logger.Debug("Created root node object. Getting subkeys.");

            var keys = this.GetSubKeysAndValues(this.Root);

            this.Root.SubKeys.AddRange(keys);

            this.Logger.Debug("Hive processing complete!");

            //All processing is complete, so we do some tests to see if we really saw everything
            if (this.RecoverDeleted && this.HiveLength() != this.TotalBytesRead)
            {
                var remainingHive = this.ReadBytesFromHive(this.TotalBytesRead, (int) (this.HiveLength() - this.TotalBytesRead));
                ...
                if (this.Header.Length != this.TotalBytesRead - 0x1000)
                {
                    this.Logger.Warn( //ncrunch: no coverage
                        "Hive length (0x{0:x}) does not equal bytes read (0x{1:x})!! Check the end of the hive for erroneous data",
                        this.HiveLength(), this.TotalBytesRead);
                }
            }

            if (this.RecoverDeleted)
            {
                this.BuildDeletedRegistryKeys();
            }

            if (this.FlushRecordListsAfterParse)
            {
                this.Logger.Debug("Flushing record lists...");
                this.ListRecords.Clear();

                var toRemove = this.CellRecords.Where(pair => pair.Value is NkCellRecord || pair.Value is VkCellRecord)
                    .Select(pair => pair.Key)
                    .ToList();

                foreach (var key in toRemove)
                {
                    this.CellRecords.Remove(key);
                }
            }
        ...
        public HashSet<string> ExpandKeyPath(string wildCardPath)
        {
            ...
            if (wildCardPath.ToUpperInvariant().StartsWith(this.Root.KeyName.ToUpperInvariant()) == false)
            {
                //ensure things look proper for our matching later
                wildCardPath = $"{this.Root.KeyName}\\{wildCardPath}";
            }

            wildCardPath = wildCardPath.ToLowerInvariant();

            if (this._keyPathKeyMap.ContainsKey(wildCardPath))
            {
                keyPaths.Add(wildCardPath);
            }
            ...
            foreach (var key in this._keyPathKeyMap.Keys)
            {
        ...
        {
            this.Logger.Debug("Associating deleted keys and values...");

            var unreferencedNkCells = this.CellRecords.Where(t => t.Value.IsReferenced == false && t.Value is NkCellRecord);
            ...
            foreach (var unreferencedNkCell in unreferencedNkCells)
            {
                try
                {
                    ...
                    if (regKey.NkRecord.ValueListCellIndex > 0)
                    {
                        ...
                        var size = this.ReadBytesFromHive(regKey.NkRecord.ValueListCellIndex + 4096, 4);
                        ...
                        try
                        {
                            var rawData = this.ReadBytesFromHive(regKey.NkRecord.ValueListCellIndex + 4096,
                                (int) sizeNum);
...
//ncrunch: no coverage
                            ...
                            while (offsetIndex < offsetList.Data.Length)
                            {
                                ...
                                this.Logger.Trace("Got value offset 0x{0:X}", os);
                  ...
                  //  var valOffsetIndex = 0;
                    ...
                    foreach (var valueOffset in nk.ValueOffsets)
                    {
                        if (this.CellRecords.ContainsKey((long) valueOffset))
                        {
                            ...
                            var val = this.CellRecords[(long) valueOffset] as VkCellRecord;
...
//ncrunch: no coverage
            ...
            this.Logger.Debug("Building tree of key/subkeys for deleted keys");
            ...
            this.Logger.Debug("Associating top level deleted keys to active Registry keys");
            ...
            foreach (var deletedRegistryKey in deletedRegistryKeys)
            {
                if (this.CellRecords.ContainsKey(deletedRegistryKey.Value.NkRecord.ParentCellIndex))
                {
                    //an parent key has been located, so get it
                    var parentNk = this.CellRecords[deletedRegistryKey.Value.NkRecord.ParentCellIndex] as NkCellRecord;
                    ...
                    if (parentNk.IsReferenced && parentNk.IsFree == false)
                    {
                        ...
                        var pk = this.GetKey(deletedRegistryKey.Value.NkRecord.ParentCellIndex);
                        ...
                        this.UpdateChildPaths(deletedRegistryKey.Value);

                        //add a copy of deletedRegistryKey under its original parent
                        pk.SubKeys.Add(deletedRegistryKey.Value);

                        this._relativeOffsetKeyMap.Add(deletedRegistryKey.Value.NkRecord.RelativeOffset,
                            deletedRegistryKey.Value);

                        if (this._keyPathKeyMap.ContainsKey(deletedRegistryKey.Value.KeyPath.ToLowerInvariant()) == false)
                        {
                            this._keyPathKeyMap.Add(deletedRegistryKey.Value.KeyPath.ToLowerInvariant(),
                                deletedRegistryKey.Value);
                        }
            ...
            this.DeletedRegistryKeys = deletedRegistryKeys.Values.ToList();

            var unreferencedVk = this.CellRecords.Where(t => t.Value.IsReferenced == false && t.Value is VkCellRecord);
            ...
            foreach (var keyValuePair in unreferencedVk)
            {
                if (associatedVkRecordOffsets.Contains(keyValuePair.Key) == false)
                {
                    ...
                    this.UnassociatedRegistryValues.Add(val);
                }
            }
        }
        ...
        private void UpdateChildPaths(RegistryKey key)
        {
            ...
            foreach (var sk in key.SubKeys)
            {
                ...
                this._relativeOffsetKeyMap.Add(sk.NkRecord.RelativeOffset, sk);

                if (this._keyPathKeyMap.ContainsKey(sk.KeyPath.ToLowerInvariant()) == false)
                {
                    this._keyPathKeyMap.Add(sk.KeyPath.ToLowerInvariant(), sk);
                }

                this.UpdateChildPaths(sk);
            }
        }
        ...
        public HiveMetadata Verify()
        {
            ...
            {
                var hbinSig = BitConverter.ToUInt32(this.ReadBytesFromHive(offset, 4), 0);
                ...
                var hbinSize = BitConverter.ToUInt32(this.ReadBytesFromHive(offset + 8, 4), 0);
        ...
        {
            foreach (var registryKey in this._keyPathKeyMap)
