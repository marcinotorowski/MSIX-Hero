namespace External.EricZimmerman.Registry
{
    ...
    public class RegistrySkeleton
    {
        ...
        public RegistrySkeleton(RegistryHive hive)
        {
            ...
            this._hive = hive;
            this._keys = new List<SkeletonKeyRoot>();
        }

        public ReadOnlyCollection<SkeletonKeyRoot> Keys => this._keys.AsReadOnly();
        ...
        {
            var hiveKey = this._hive.GetKey(key.KeyPath);
            ...
            if (key.KeyPath.StartsWith(this._hive.Root.KeyName) == false)
            {
                var newKeyPath = $"{this._hive.Root.KeyName}\\{key.KeyPath}";
                var tempKey = new SkeletonKeyRoot(newKeyPath, key.AddValues, key.Recursive);
                key = tempKey;
            }

            var intKey = this._keys.SingleOrDefault(t => t.KeyPath == key.KeyPath);
            ...
            if (intKey == null)
            {
                ...
                if (key.Recursive)
                {
                    ...
                    var subs = this.GetSubkeyNames(hiveKey);

                    foreach (var sub in subs)
                    {
                        var subsk = new SkeletonKeyRoot(sub, true, false);
                        this._keys.Add(subsk);
                    }
                }
            }
        ...
        private List<string> GetSubkeyNames(RegistryKey key)
        {
            ...
            {
                l.AddRange(this.GetSubkeyNames(registryKey));
        ...
        private byte[] GetEmptyHbin(int size)
        {
            ...
            BitConverter.GetBytes(this._relativeOffset).CopyTo(newHbin, 0x4);
            this._relativeOffset += size;
        ...
        public bool Write(string outHive)
        {
            ...
            this._hbin = this._hbin.Concat(this.GetEmptyHbin(0x1000)).ToArray();


            var treeKey = this.BuildKeyTree();

            var parentOffset = this.ProcessSkeletonTree(treeKey); //always include keys/values for now

            //mark any remaining hbin as free
            var freeSize = this._hbin.Length - this._currentOffsetInHbin;
            if (freeSize > 0)
            {
                BitConverter.GetBytes(freeSize).CopyTo(this._hbin, this._currentOffsetInHbin);
            }

            //work is done, get header, update rootcelloffset, adjust its length to match new hbin length, and write it out

            var headerBytes = this._hive.ReadBytesFromHive(0, 0x1000);

            BitConverter.GetBytes(this._hbin.Length).CopyTo(headerBytes, 0x28);
            ...
            var outBytes = headerBytes.Concat(this._hbin).ToArray();
        ...
        {
            if (this._currentOffsetInHbin + recordSize > this._hbin.Length)
            {
                ...
                this._currentOffsetInHbin = this._hbin.Length;
                ...
                //add more space
                this._hbin = this._hbin.Concat(this.GetEmptyHbin(hbinSize)).ToArray();

                //move pointer to next usable space
                this._currentOffsetInHbin += 0x20;
            }
        }

        private int ProcessSkRecord(uint skIndex)
        {
            if (!this._hive.CellRecords.ContainsKey(skIndex))
            {
                return 0;
            }

            var sk = this._hive.CellRecords[skIndex] as SkCellRecord;

            if (this._skMap.ContainsKey(sk.RelativeOffset))
            {
                //sk is already in _hbin
                return this._skMap[sk.RelativeOffset];
            }

            this.CheckhbinSize(sk.RawBytes.Length);
            sk.RawBytes.CopyTo(this._hbin, this._currentOffsetInHbin);

            var skOffset = this._currentOffsetInHbin;
            this._skMap.Add(sk.RelativeOffset, skOffset);
            this._currentOffsetInHbin += sk.RawBytes.Length;
            return skOffset;
        }
...
//            return _currentOffsetInHbin;
        ...
        private int ProcessValue(KeyValue value)
        {
            ...
            if ((value.VkRecord.DataLength & dwordSignMask) != dwordSignMask)
            {
                ...
                if (value.VkRecord.DataLength > 16344)
                {
                    ...
                    foreach (var chunk in chunks)
                    {
                        ...
                        this.CheckhbinSize(toWrite.Length);
                        toWrite.CopyTo(this._hbin, this._currentOffsetInHbin);

                        dbOffsets.Add(this._currentOffsetInHbin);

                        this._currentOffsetInHbin += toWrite.Length;
                    }
                    ...
                    this.CheckhbinSize(offsetList.Length);

                    var offsetOffset = this._currentOffsetInHbin;

                    offsetList.CopyTo(this._hbin, offsetOffset);
                    this._currentOffsetInHbin += offsetList.Length;
                    ...
                    var dbOffset = this._currentOffsetInHbin;
                    this.CheckhbinSize(dbRaw.Length);
                    dbRaw.CopyTo(this._hbin, dbOffset);

                    this._currentOffsetInHbin += dbRaw.Length;
                ...
                else
                {
                    ...
                    this.CheckhbinSize(datarawBytes.Length);
                    datarawBytes.CopyTo(this._hbin, this._currentOffsetInHbin);

                    BitConverter.GetBytes(this._currentOffsetInHbin).CopyTo(vkBytes, ValueDataOffset);

                    this._currentOffsetInHbin += datarawBytes.Length;
                }
            }

            this.CheckhbinSize(vkBytes.Length);

            var vkOffset = this._currentOffsetInHbin;
            ...
            vkBytes.CopyTo(this._hbin, vkOffset);

            this._currentOffsetInHbin += vkBytes.Length;
        ...
        {
            var skOffset = this.ProcessSkRecord(key.NkRecord.SecurityCellIndex);

            var classOffset = this.ProcessClassCell(key.NkRecord.ClassCellIndex);

            //do we have enough room left to place our NK?
            this.CheckhbinSize(key.NkRecord.RawBytes.Length);

            //this is where we will be placing our record
            var nkOffset = this._currentOffsetInHbin;

            //move our pointer to the beginning of free space for any subsequent records
            this.CheckhbinSize(key.NkRecord.RawBytes.Length);
            this._currentOffsetInHbin += key.NkRecord.RawBytes.Length;
            ...
            if (addValues)
            {
                ...
                {
                    var valOffset = this.ProcessValue(keyValue);

                    valueOffsets.Add(valOffset);
                }

                var valueListBytes = this.BuildValueList(valueOffsets);
                this.CheckhbinSize(valueListBytes.Length);
                valueListBytes.CopyTo(this._hbin, this._currentOffsetInHbin);

                //update NK record to point to our new list of values and update the value count
                BitConverter.GetBytes(this._currentOffsetInHbin).CopyTo(nkBytes, ValueListCellIndex);
                BitConverter.GetBytes(valueOffsets.Count).CopyTo(nkBytes, ValueCountIndex);

                this._currentOffsetInHbin += valueListBytes.Length;
            }
            ...
            if (addSubkeys)
            {
                ...
                foreach (var registryKey in key.SubKeys)
                {
                    var subkeyOffset = this.ProcessKey(registryKey, nkOffset, addValues, true);
                ...
                var subkeyListBytes = this.BuildlfList(subkeyOffsets);

                this.CheckhbinSize(subkeyListBytes.RawBytes.Length);
                subkeyListBytes.RawBytes.CopyTo(this._hbin, this._currentOffsetInHbin);

                //update nk record pointers to subkeylist and subkey count
                BitConverter.GetBytes(this._currentOffsetInHbin).CopyTo(nkBytes, SubkeyListsStableCellIndex);
                ...
                this._currentOffsetInHbin += subkeyListBytes.RawBytes.Length;
            }
            ...
            this.CheckhbinSize(nkBytes.Length);
            nkBytes.CopyTo(this._hbin, nkOffset);
        ...
        private int ProcessSkeletonTree(SkeletonKey treeKey)
        {
            ...
            {
                this.ProcessSkeletonTree(skeletonKey);
            }

            var key = this._hive.GetKey(treeKey.KeyPath);

            var parentOffset = this.ProcessKey(key, -1, treeKey.AddValues, true);
        ...
        private SkeletonKey BuildKeyTree()
        {
            ...
            foreach (var keyRoot in this._keys)
            {
                var current = root;
    ...
    public class SkeletonKey
    {
        public SkeletonKey(string keyPath, string keyName, bool addValues)
        {
            ...
            this.AddValues = addValues;
            this.Subkeys = new List<SkeletonKey>();
        }
